    ...
    public override void OnGUI (Rect position, SerializedProperty prop, GUIContent label) {
        ...
        try {
            if (prop.propertyType == SerializedPropertyType.Integer) {
                ...
                throw new UnityException("must be int or float property to use with NotMoreThan");
            }
        }
        catch (UnityException e) {
            throw new UnityException("error on NotMoreThan attribute of property " + prop.name + "\n" + e.ToString());
        }
    }
}