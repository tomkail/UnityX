...
namespace UnityX.Geometry {
	public class PolygonEditorHandles {
		...
		Matrix4x4 directionMatrix => Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one) * offsetMatrix;
		...
		bool moveMode => editing && Event.current.shift;
		...
		bool isEditingPoint => editingPointIndex != -1;
		...
		Plane plane => new(directionMatrix.MultiplyVector(Vector3.forward), matrix.MultiplyPoint3x4(Vector3.zero));
