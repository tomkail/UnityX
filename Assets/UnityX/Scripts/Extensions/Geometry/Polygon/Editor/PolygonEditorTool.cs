...
namespace UnityX.Geometry {
	class PolygonEditorTool : EditorTool {
		...
		[SerializeField] Texture2D m_ToolIcon = null;

		GUIContent m_IconContent;


		public static bool editable = true;
		public static bool editing => ToolManager.activeToolType == typeof(PolygonEditorTool);

		public static bool showSceneViewTools = false;


		bool usingHandOrbitTool => Tools.viewTool == ViewTool.Orbit && Event.current.alt;

		static bool moveMode => Event.current.shift;
		...
		static bool isEditingPoint => activeInstance != null && editingPointIndex != -1;

		static PolygonEditorInstance targetInstance => activeInstance ?? hoveredInstance;
