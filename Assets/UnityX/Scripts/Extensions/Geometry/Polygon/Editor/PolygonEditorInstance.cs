...
namespace UnityX.Geometry {
	public class PolygonEditorInstance {
		...
		public bool Valid => polygon != null;
		...
		public Matrix4x4 directionMatrix => Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one) * offsetMatrix;

		public Plane plane => new(directionMatrix.MultiplyVector(Vector3.forward), matrix.MultiplyPoint3x4(Vector3.zero));
		...
		public PolygonEditorInstance(Transform transform) {
			...
			PolygonInvalidFunc = (Polygon polygon) => { return polygon == null || polygon.vertices.Length < (closed ? 3 : 2); };
		}
