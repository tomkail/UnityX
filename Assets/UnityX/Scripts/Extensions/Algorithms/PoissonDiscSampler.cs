...
public class PoissonDiscSampler
{
    ...
    public IEnumerable<Vector2> Samples() {
        ...
        return Samples(new Vector2(Random.value * rect.width, Random.value * rect.height));
    }

    // This can be used to "seed" the distribution so that it begins from a certain point.
    public IEnumerable<Vector2> Samples(Vector2 firstSample) {
        if(!rect.Contains(firstSample)) Debug.LogWarning($"First sample {firstSample} is not contained in rect {rect}");
        ...
        while (activeSamples.Count > 0) {
            ...
            bool found = false;
            float startAngle = 2 * Mathf.PI * Random.value;
            float angleIncrement = 2 * Mathf.PI / k; // Evenly spaced angle increments
            for (int j = 0; j < k; ++j) {
                float angle = startAngle + j * angleIncrement;
